...
public class BorrowDAO {
    ...
    private static final String GetBorrowDetail = "SELECT bd.BorrowID, bd.BookID, b.Title, bd.Quantity, bd.LostQty  " +
    ...
    public List<BorrowDetailDTO> getBorrowDetails(int borrowId_input) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int lostQty = rs.getInt("LostQty"); // lấy thêm lostQty
                list.add(new BorrowDetailDTO(borrowId, bookId, title, quantity, lostQty));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    String sql = "UPDATE Borrows SET Status = 'Lost' WHERE BorrowID = ?";
    ...
    public boolean updateLostQty(int borrowId, int bookId, int lostQty) {
    String sql = "UPDATE BorrowDetails SET LostQty = ? WHERE BorrowID = ? AND BookID = ?";
    try (Connection conn = DBUtils.getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setInt(1, lostQty);
        ps.setInt(2, borrowId);
