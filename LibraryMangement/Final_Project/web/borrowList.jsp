...
<c:choose>
                            ...
                            <c:forEach var="borrow" items="${listBorrows}" varStatus="status">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <c:if test="${empty borrow.returnDate and borrow.status ne 'Lost'}">
                                        ...
                                        <c:if test="${empty borrow.returnDate and borrow.status ne 'Lost'}">
                                            <form action="BorrowController" method="get" style="display:inline;">
                                                <input type="hidden" name="action" value="markLostForm" />
                                                <input type="hidden" name="borrowId" value="${borrow.borrowId}" />
                                                <button type="submit" class="btn btn-danger btn-sm">Mark Lost</button>
                                            </form>
                                        </c:if>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <form action="BorrowController" method="post" onsubmit="return confirm('Confirm marking books as lost?');">
                <div class="modal-header">
                    <h5 class="modal-title" id="markLostModalLabel">Mark Lost Books</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="lostBooksContent">
                    Loading...
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="action" value="markLost" />
                    <input type="hidden" name="borrowId" id="lostBorrowId" />
