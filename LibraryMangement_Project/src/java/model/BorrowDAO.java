...
public boolean markLost(int borrowId) throws Exception {
    String updateBorrowSql = "UPDATE Borrows SET Status = 'Lost', ReturnDate = ? WHERE BorrowID = ?";
    String insertFineSql = "INSERT INTO Fines (BorrowID, Amount, Reason, StatusCode, CreatedAt) VALUES (?, ?, ?, ?, ?)";

    try (Connection con = DBUtils.getConnection()) {
        con.setAutoCommit(false); // Transaction

        // 1. Cập nhật trạng thái Borrow
        try (PreparedStatement pst = con.prepareStatement(updateBorrowSql)) {
        ...
        // 2. Tạo Fine do mất sách
        try (PreparedStatement pst = con.prepareStatement(insertFineSql)) {
            pst.setInt(1, borrowId);
            pst.setDouble(2, 100000); // Ví dụ: phạt mất sách 100k
            pst.setString(3, "LOST"); // Reason = ReasonCode trong bảng FineReasons
            pst.setString(4, "Unpaid"); // hoặc lấy từ enum/table
