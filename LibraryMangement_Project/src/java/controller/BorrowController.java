...
public class BorrowController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            else if (action.equals("markLost")) {
                url = handleMarkLost(request, response);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    //phong them
    private String handleMarkLost(HttpServletRequest request, HttpServletResponse response) {
    if (!GeneralMethod.isAdmin(request)) {
        GeneralMethod.getAccessDenied(request, "You do not have permission.");
        return "welcome.jsp";
    }

    try {
        int borrowId = Integer.parseInt(request.getParameter("borrowId"));
        boolean success = brdao.markLost(borrowId);
        if (success) {
            request.setAttribute("message", "Book marked as lost and fine has been issued.");
        } else {
